    ...
    for e in apex:
        ...
        if diameter > min_diameter:
            ...
            if euclidean:
                ...
                d = dist(a, b)*1.6 #convert lengths from pixels to µm; 1 pixel = 1.6 µm
            else:
                d = gp[car][cdr]["weight"]*1.6 #convert lengths from pixels to µm; 1 pixel = 1.6 µm
            if verbose:
                out.append((a, b, d))
            else:
                out.append(d)
    ...
    for edge in gp.edges():
        ...
        if gp.degree[e0]==3 and gp.degree[e1]==3:
            ...
            if da > min_diameter or db > min_diameter:
                out.append(gp[e0][e1]['weight']*1.6) #convert lengths from pixels to µm; 1 pixel = 1.6 µm
    return out
...
def l_tot(g, n=1, k=0, min_diameter=0):
    ...
    Args:
        ...
        k (int): index of the sector considered
        min_diameter (int): minimum distance of node to center
    ...
    for (a,b) in gp.edges():
        ...
        if da > min_diameter or db > min_diameter:
            if (ta >= 2*np.pi*k/n and ta < 2*np.pi*(k+1)/n) or (tb >= 2*np.pi*k/n and tb < 2*np.pi*(k+1)/n):
                l += gp[a][b]['weight']*1.6 #convert lengths from pixels to µm; 1 pixel = 1.6 µm
